...
impl Solution {
    ...
    // consider which balloon to be bursted last in range i to j, both exclusive.
    // 3 1 5 8 ──▶ 1 │ 3 1 5 8 │ 1
    //
    // 0   1 2 3 4   5
    // 1 │ 3 1 5 8 │ 1
    ...
    // f(N,i,j) = f(0,5) = ?
    //
    // f(N,1,4) which balloon to be bursted last? 1 or 5?
    //
    // if it is 1, 3 * 1 * 8 + f(N, 1, 2) + f(N, 2,4) => 24 + f(N,1,2) + f(N,2,4)
    // f(N,1,2) = 0, no balloons bewteen, no gains
    // f(N,2,4) = 1*5*8 + f(N,2,3) + f(N,3,4) = 40 + 0 + 0 = 40
    //
    // Hence, if it is 1, the max gains is 24 + 0 + 40 = 64
    //
    // if it is 5, 3 * 5 * 8 + f(N, 1, 3) + f(N, 3,4) => 120 + f(N,1,3) + f(N,3,4)
    // f(N,3,4) = 0, no balloons bewteen, no gains
    // f(N,1,3) = 3*1*5 = 15
    //
    // Hence, if it is 5, the max gains is 120 + 15 + 0 = 135
    //
    //
    // Hence, f(N,1,4) = MAX(54,135) = 135
    //
    // g(0,5) = MAX(
    //  1*3*1+g(0,1)+g(1,5),
    //  3*1*5+g(0,2)+g(2,5),
    ...
    //
    // For g(0,2) = g(1│3│1) = 3
    // For g(1,3) = g(3│1│5) = 15
    // For g(2,4) = g(1│5│8) = 40
    // For g(3,5) = g(5│8│1) = 40
    //
    //       1   3   1   5   8   1
    //       0   1   2   3   4   5
    //     ┌──────────────────────
    ...
    // 1 5 │ 0   0   0   0   0   0
    //
    // For g(0,3) = g({1│3 1│5) = MAX(
    //  1*3*5+g(0,1)+g(1,3)
    //  1*1*5+g(0,2)+g(2,3)
    // ) = MAX(15+0+15, 5+3+0) = 30
    //
    // For g(1,4) = g({3│1 5│8}) = MAX(
    //  3*1*8+g(1,2)+g(2,4),
    //  3*5*8+g(1,3)+g(3,4),
    // ) = MAX(24+0+40, 120+15+0) = 135
    //
    // For g(2,5) = g(1│5 8│1) = MAX(
    //  1*5*1+g(2,3)+g(3,5),
    //  1*8*1+g(2,4)+g(4,5),
    // ) = MAX(5+0+40,8+40+0)=48
    //
    //       1   3   1   5   8   1
    //       0   1   2   3   4   5
    //     ┌──────────────────────
    ...
    // 1 5 │ 0   0   0   0   0   0
    //
    // For g(0,4) = g({1│3 1 5│8}) = MAX(
    //  1*3*8+g(0,1)+g(1,4),
    //  1*1*8+g(0,2)+g(2,4),
    //  1*5*8+g(0,3)+g(3,4),
    // ) = MAX(24+0+135, 8+3+40, 40+30+0) = 159
    //
    // For g(1,5) = g({3│1 5 8│1}) =MAX(
    //  3*1*1+g(1,2)+g(2,5),
    //  3*5*1+g(1,3)+g(3,5),
    //  3*8*1+g(1,4)+g(4,5),
    // ) = MAX(3+0+48, 15+15+40, 24+135+0) = 159
    //
    //       1   3   1   5   8   1
    //       0   1   2   3   4   5
    //     ┌──────────────────────
    // 1 0 │ 0   0   3   30  159 ❓
    // 3 1 │ 0   0   0   15  135 159
    // 1 2 │ 0   0   0   0   40  48
    // 5 3 │ 0   0   0   0   0   40
    // 8 4 │ 0   0   0   0   0   0
    // 1 5 │ 0   0   0   0   0   0
    //
    // For g(0,5) = g(1│3 1 5 8│1) = MAX(
    //  1*3*1+g(0,1)+g(1,5),
    //  1*1*1+g(0,2)+g(2,5),
    //  1*5*1+g(0,3)+g(3,5),
    //  1*8*1+g(0,4)+g(4,5),
    // ) = MAX(3+0+159, 1+3+48, 5+30+40, 8+159+0) = 167
    //
    //       1   3   1   5   8   1
    //       0   1   2   3   4   5
    //     ┌──────────────────────
    // 1 0 │ 0   0   3   30  159 167 ✅
    // 3 1 │ 0   0   0   15  135 159
    // 1 2 │ 0   0   0   0   40  48
    // 5 3 │ 0   0   0   0   0   40
    // 8 4 │ 0   0   0   0   0   0
    ...
    pub fn max_coins_dp(nums: Vec<i32>) -> i32 {
        ...
        let nums = [vec![1], nums, vec![1]].concat();
        // println!("nums={:?}", nums);
            ...
            for i in 0..dp.len() {
                for j in 0..dp[0].len() {
                    if j > i + 1 && j - i == k + 1 {
                        for n in 1..=k {
                            // println!(
                            //     "i={i} j={j} k={k} n={n} nums[i]={} nums[i+n]={} nums[j]={}",
                            //     nums[i],
                            //     nums[i + n],
                            //     nums[j]
                            // );
                            let v = nums[i] * nums[i + n] * nums[j] + dp[i][i + n] + dp[i + n][j];
                            dp[i][j] = dp[i][j].max(v);
                        }
                    }
                }
