...
fn test_example_4() {
    // invalid order
    let words = vec!["abcde".to_string(), "abc".to_string()];
...
fn test_example_5() {
    let words = vec![
        "mnop", "nopq", "opqr", "pqrs", "qrst", "rstu", "stuv", "tuvw", "uvwx", "vwxy", "wxyz",
        "xyz", "yz", "z", "mnopqr", "nopqrs", "opqrst", "pqrstu", "qrstuv", "rstuvw", "stuvwx",
        "tuvwxy", "uvwxyz", "vwxyz", "wxyza", "xyzab", "yzabc", "zabcd",
    ];
    let words: Vec<_> = words.into_iter().map(|x| x.to_string()).collect();
