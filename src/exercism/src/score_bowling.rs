...
/// key idea from : https://exercism.org/tracks/rust/exercises/bowling/solutions/potatosalad
/// basic thinking model:
/// put all pins in a Vec and then use variables to partition this Vec.
...
impl BowlingGame {
    ...
    pub fn roll(&mut self, pins: u16) -> Result<(), Error> {
        if pins > 10 || self.second_throw_in_frame && self.rolls.last().unwrap() + pins > 10 {
        ...
        self.second_throw_in_frame = if pins == 10 {
            false
        } else {
            !self.second_throw_in_frame
        };

        Ok(())
    }

    /// basic thinking model: if we can not get the final score, the game is not over.
    pub fn score(&self) -> Option<u16> {
        let mut total = 0;
        let mut n = 0;
        for _ in 0..10 {
            // key observation
            if let (Some(&first), Some(&second)) = (self.rolls.get(n), self.rolls.get(n + 1)) {
                total += first + second;
                if first == 10 || first + second == 10 {
                    if let Some(third) = self.rolls.get(n + 2) {
                        total += third;
                    } else {
                        return None;
                    }
